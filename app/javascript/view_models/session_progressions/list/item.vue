...
.entity-list-item-wrapper.session_progression-list-item-wrapper
  ...
  .entity-list-item-actions(v-if='!confirming_destroy && showing_actions')
    .ui.four.column.grid
      .column(@click='$emit("edit")')
        router-link.ui.basic.fluid.green.icon.button(:to='session_progression.path()')
          i.eye.icon
          |  View
  ...
  .entity-list-item.session_progression-list-item(v-if='!confirming_destroy && !showing_actions')
    session_progressions-list-item-contents(:session_progression='session_progression')
